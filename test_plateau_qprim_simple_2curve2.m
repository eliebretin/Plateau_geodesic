...
epsilon =2/N;
  ...
  vv = VideoWriter(['test_plateau_2dcircle2.avi'],'Motion JPEG AVI');
   vv.Quality = 90;
   open(vv);
...
gamma1_1f =  @(theta) R*cos(theta)+0.5*(1 + 0.1*cos(4*theta)) ;
gamma1_2f = @(theta) R*sin(theta)+0.5 ;
gamma1_3f = @(theta) 0.3*(1 - 0.1*cos(4*theta)) ;


gamma2_1f =  @(theta) R*cos(theta).*(1 + 0.2*cos(8*theta))+0.5 ;
gamma2_2f = @(theta)  R*sin(theta).*(1 + 0.2*cos(8*theta))+0.5 ;
gamma2_3f = @(theta) 0.6*theta.^0;
...
lighting gouraud
    ...
    title(['n = ',num2str(i)]);
    name_fig = ['Test_plateau_2circle_2_',num2str(j_vec),'.eps']; 
