...
epsilon =2/N;
  ...
  vv = VideoWriter(['test_plateau_2dcircle4_.avi'],'Motion JPEG AVI');
   vv.Quality = 90;
   open(vv);
...
gamma1_1f =  @(theta) 1.2*R*cos(theta)+0.5 ;
gamma1_2f = @(theta) 0.9*R*sin(theta)+0.5 ;
gamma1_3f = @(theta) 0.5*theta.^0 ;


gamma2_1f =  @(theta) 0.9*R*sin(theta)+0.5 ;
gamma2_2f = @(theta) 0.5*theta.^0 - 0.025*1*R*cos(theta);
...
Ge2 = compute_geodesic_l_modif(U,Mreg,gamma1_1,gamma1_2,gamma1_3,gamma2_1,gamma2_2,gamma2_3,epsilon);
...
lighting gouraud
         ...
         view(-250,30);
    axis([0.05,0.95,0.05,0.95,0.05,0.95])
     pause(0.1)
...
lighting gouraud
    ...
    title(['n = ',num2str(i)]);
    name_fig = ['Test_plateau_2circle_4_',num2str(j_vec),'.eps']; 
